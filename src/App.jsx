...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (fromAboutScrollUp) {
      // ── Set progress refs SYNCHRONOUSLY so the canvas + spring loop
      //    see the end position from the very first tick. If we wait until
      //    the rAF below, the render loops fire one frame at progress=0
      //    and play backwards through every frame — looks like a rewind.
      ...
      requestAnimationFrame(() => {
        ...
        // Re-assert in case any scroll handler clobbered them between ticks
        rawProgressRef.current = 0.995
        progressRef.current = 0.995
      })
